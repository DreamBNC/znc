...
#ifndef HAVE_LIBSSL
    CUtils::PrintError("WZNC was not compiled with ssl support.");
    return false;
#else
...
CString CZNC::MakeConfigHeader() {
    return "// WARNING\n"
           "//\n"
           "// Do NOT edit this file while WZNC is running!\n"
...
// Listen
    ...
    do {
        ...
        while (true) {
            ...
            if (uListenPort == 6667) {
                ...
                CUtils::PrintStatus(false,
                                    "use WZNC's web interface, you might want "
                                    "to use another port.");
    ...
    if (CUtils::GetBoolInput("Set up a network?", true)) {
        ...
        do {
            CUtils::GetInput("Name", sNetwork, "Rizon");
    ...
    do {
        ...
        if (CFile::Exists(m_sConfigFile)) {
            if (!File.TryExLock(m_sConfigFile)) {
                CUtils::PrintStatus(false,
                                    "WZNC is currently running on this config.");
                bFileOK = false;
            } else {
                File.Close();
    ...
    CUtils::PrintMessage("");
    CUtils::PrintMessage(
        "To connect to this WZNC you need to connect to it as your IRC server",
        true);
    CUtils::PrintMessage(
    ...
    if (bWantLaunch) {
        bWantLaunch = CUtils::GetBoolInput("Would you like to launch WZNC now?", true);
    }
    return bWantLaunch;
}
...
bool CZNC::ReadConfig(CConfig& config, CString& sError) {
    ...
    if (!CFile::Exists(m_sConfigFile)) {
        ...
        CUtils::PrintMessage(
            "Restart WZNC with the --makeconf option if you wish to create this "
            "config.");
        return false;
    }
    ...
    if (!pFile->TryExLock()) {
        sError = "WZNC is already running on this config.";
        CUtils::PrintStatus(false, sError);
        delete pFile;
        return false;
    }
    ...
    if (tSavedVersion < tCurrentVersion) {
        CUtils::PrintMessage("Found old config from WZNC " + sSavedVersion +
        ...
        CUtils::PrintError("Config was saved from ZNC " + sSavedVersion +
                           ". It may or may not work with current WZNC " +
                           GetVersion());
    }
